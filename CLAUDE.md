...
- **Run end-of-feature checks** (see Rust section) before declaring work done.
...
Compiler/lint errors during a multi-step edit are expected and should be ignored. Only run the end-of-feature checks (see Rust section) once all planned changes are complete.
...
Once a feature is finished, run these checks in order:

1. `cargo clippy --all-targets` — Clippy/compiler errors
